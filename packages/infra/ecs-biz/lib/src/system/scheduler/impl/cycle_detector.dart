...
import 'compute_scheduler_impl.dart';

class CycleDetector {
  /// 核心入口
  static Map<PropertyId, ComputeError> analyzeErrors(
    List<PropertyId> allDirties,
    ...
    final Map<PropertyId, List<PropertyId>> graph = {};
    final Map<PropertyId, int> inDegree = {};
    ...
    // 5. 标记结果
    final Map<PropertyId, ComputeError> result = {};
    ...
    for (var scc in sccs) {
      ...
      final errorType = isCycle
          ? ComputeError.cycleDependencies
          : ComputeError.referenceInvalid;
      for (var node in scc) {
  ...
  static List<List<PropertyId>> _findSCCs(
    List<PropertyId> nodes,
    Set<PropertyId> nodeSet,
    Map<PropertyId, List<PropertyId>> graph,
  ) {
    int indexCounter = 0;
    final Map<PropertyId, int> indices = {};
    final Map<PropertyId, int> lowLinks = {};
    final List<PropertyId> stack = [];
    final Set<PropertyId> onStack = {};
    final List<List<PropertyId>> sccs = [];

    void strongConnect(PropertyId v) {
      ...
      if (lowLinks[v] == indices[v]) {
        final List<PropertyId> currentSCC = [];
        PropertyId w;
        do {
          w = stack.removeLast();
          onStack.remove(w);
